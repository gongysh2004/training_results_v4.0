...
or to get the trainset in both standard and packed format (i.e. more than one sequence are packed into trainng sample to minimize padding). 
...
seq -w 0000 04319 | xargs -I{} --max-args 1 --max-procs 30 python input_preprocessing/hdf5_md5.py \
    --input_hdf5 path/to/varlength/shards/part_{}_of_04320.hdf5 | tee 4320_shards_varlength.chk
sort 4320_shards_varlength.chk
```
Checksums after shuffling with default seed (24)
seq -w 0000 04319 | xargs -I{} --max-args 1 --max-procs 30 python input_preprocessing/hdf5_md5.py \
    --input_hdf5 path/to/varlength shuffled/shards/part_{}_of_04320.hdf5 | tee 4320_shards_varlength_shuffled-24.chk
sort 4320_shards_varlength_shuffled-24.chk
...
CONT=mlperf-nvidia:language_model DATADIR_PHASE2=<path/to/4320_shards_varlength/dir> EVALDIR=<path/to/eval_varlength/dir> CHECKPOINTDIR=<path/to/result/checkpointdir> CHECKPOINTDIR_PHASE1=<path/to/pytorch/ckpt/dir> ./run_with_docker.sh
```
