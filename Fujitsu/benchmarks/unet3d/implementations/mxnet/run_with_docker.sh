...
#    "--volume=$(realpath runtime/setup.py):/workspace/unet3d/runtime/setup.py"
#    "--volume=$(realpath main.py):/workspace/unet3d/main.py"
    "--volume=$(realpath run_and_time_mod.sh):/workspace/unet3d/run_and_time_mod.sh"
)
...
#mllog_event(key=constants.CACHE_CLEAR, value=True)"
        ...
        docker exec -it "${_config_env[@]}" "${_cont_name}" \
	       mpirun --allow-run-as-root --bind-to none -np ${DGXNGPU} ./run_and_time_mod.sh
